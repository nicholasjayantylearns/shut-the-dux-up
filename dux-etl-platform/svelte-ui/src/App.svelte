<script>
    ...
    import InsightsCarousel from "./lib/dux-showcase/InsightsCarousel.svelte";

    // Routing state
    let currentPage = "chat"; // "chat" or "showcase"
...
<!-- Navigation -->
<nav class="bg-white shadow-sm border-b border-indigo-200">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
            <div class="flex">
                <div class="flex-shrink-0 flex items-center">
                    <h1 class="text-xl font-bold text-indigo-600">GenAI Stack</h1>
                </div>
            </div>
            <div class="flex space-x-8">
                <button
                    class="px-3 py-2 rounded-md text-sm font-medium transition-colors {currentPage === 'chat' ? 'bg-indigo-100 text-indigo-700' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'}"
                    on:click={() => navigateTo('chat')}
                >
                    Chat Interface
                </button>
                <button
                    class="px-3 py-2 rounded-md text-sm font-medium transition-colors {currentPage === 'showcase' ? 'bg-indigo-100 text-indigo-700' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'}"
